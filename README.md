...
#### 내가 시도해본 것들은 아래와 같다.<br/>
1. ##### React에서 Nextjs로 마이그레이션
   Google Bot이 SPA인 React를 이해하지 못해 생기는 문제라고 생각했다.<br/>
   SPA에서도 Adsense 승인을 받았다 주장하는 일본어 포스트도 있었지만 증거는 없었기에 Nextjs로 마이그레이션 해봤다.<br/>
   결과는 X
2. ##### 이용약관 명시
   Adsense 문서를 읽어보면 사이트주는 사용자에게 Adsense를 차단하거나 광고를 설정할 수 있는 방법을 명시해야 한다고 나와있었다.<br/>
   그래서 이를 설명해주는 페이지와 privacy-policy, terms, about us, contact us 같은 기본이 되는 약관 페이지들을 추가했다.<br/>
   결과는 X
3. ##### 가짜 포스트들 만들기
   평균 800~1,000자 정도 되는 리그 오브 레전드 관련 포스트들을 20개 작성하였다.<br/>
   너무 번거로운 작업이라 절대 하기 싫었지만 말이다.<br/>
   결과는 X
3. ##### Google Search Console 색인 등록
   글이 있어도 색인이 등록돼있지 않다면 Google Bot은 모른다는 티스토리 포스트를 봤다.<br/>
   그래서 내 포스트 20개 중 15개의 색인이 등록되자 나는 Adsense 승인 요청을 했다.<br/>
   솔직히 여기서 될 것이라 확신했고, 정말 승인이 됐다.<br/>
승인 요청을 하면 결과를 받는 데 짧게는 1주에서 길게는 2주가 소요된다.<br/>
계속 박치기 하느라 3개월이나 소요됐다....
